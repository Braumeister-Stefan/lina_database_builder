...
def _sv(n, site, date, formula):
    ...
    NOTE: entries produced by this helper are programmatically generated by
    cycling through a fixed list of formulas and sites.  They are flagged with
    ``is_synthetic=True`` so downstream consumers can exclude them from
    frequency, collocation, or distribution analyses.
    """
    return {
        "tablet_id":          f"SV {n}",
        "site":               site,
...
def _seal(n, site, date, text):
    """Helper to generate a clay sealing entry.

    NOTE: entries produced by this helper are programmatically generated by
    cycling through a fixed list of short transliterations and sites.  They are
    flagged with ``is_synthetic=True`` so downstream consumers can exclude them
    from frequency, collocation, or distribution analyses.
    """
    return {
        "tablet_id":          f"SEAL {n}",
        "site":               site,
...
def _cer(n, site, date, text):
    """Helper to generate a ceramic inscription entry.
