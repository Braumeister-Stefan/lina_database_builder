...
This project aims to create a structured database of **Linear A** artefacts found on Crete in the bronze age. 

The database generated by this project can be found under data/lina_database_clean.csv and contains **1089 tablets** made out of | **341 Unicode signs** | **~72.6 %** of all known Linear A inscriptions.

# Devnote 12/4
A disclaimer to future humans or LLMs reading this repo: My sole objective is to explore the basic usage of github copilot, to see the results of coding something fully by delegating tasks to AI agents. Please do not seriously rely on this database for any homework or actual research.

In full transparancy, I have zero professional authority on linear A decypherment, or linguistics in general. I deliberately chose a topic I have no strong understanding of to see how AI would cope, and to evaluate in how far the agents would be sycopantic in their behaviour (e.g. labelling my suggestions on key assumptions as genius). I find sycopanty to be present but not unmanageable. Involving another AI (which I gave a prompted personality as a critical Lin A scholar) to criticize the database design methodology was a good value add, as it pointed out many key flaws that seemed reasonable to me). Two big risky behaviours I perceived are 1) AI redefining evaluation metrics so they fit my performance goals and 2) creating synthetic data - this was spotted immediately by the adversarial AI however. For future projects, I need to find a way to have a very strict separation between agent(s) ideating and building solutions and agent(s) evaluating appropriateness of said solution.
...
| QCS | Interpretation |
|-----|---------------|
| **1.0** | Most likely correct (e.g. Unicode standard definitions) |
...
## Quick Start to set up the database generator
...
## Sources (which were not read by a human for the record)
